...
- `src/research_school_radar/site_assets.py`：加载包内 HTML 外壳，并把真实 CSS/JavaScript 复制到生成站点
- `src/research_school_radar/web/templates/`：受版本控制的首页、详情页和来源页 HTML 外壳
...
- `src/research_school_radar/site_detail.py`、`site_sources_page.py` 与 `site_home_page.py`：准备经过转义的页面数据和片段；`site_components.py`、`site_calendar.py`、`site_filters.py`、`site_layout.py`、`site_paths.py` 提供可复用的展示组件，`site.py` 只协调产物生成
- `src/research_school_radar/site_home.py`：负责首页说明区块；HTML、CSS 和 JavaScript 不再存放于 Python 字符串常量中
