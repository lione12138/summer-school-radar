...
- 多学科可信来源扫描
- 来源页二级候选链接跟进
- 直接 JSON/API/结构化页面 collector，例如 IHE Delft、ELLIS、SIB；是否启用统一由 `config/sources.yaml` 的 `collector` 字段控制
...
- `src/research_school_radar/collector_ihe.py`、`collector_ellis.py` 与 `collector_sib.py`：分别实现 IHE Delft、ELLIS 和 SIB 的来源专用采集；SIB 直接读取官网嵌入的 Bioschemas `Course` / `CourseInstance`，无需浏览器；`api_sources.py` 只按 `config/sources.yaml` 注册项分发并记录健康结果
