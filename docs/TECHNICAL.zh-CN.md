...
- `src/research_school_radar/site_detail.py` 与 `site_sources_page.py`：分别负责项目详情页和来源页模板；`site_components.py`、`site_calendar.py`、`site_filters.py`、`site_layout.py`、`site_paths.py` 提供可复用的展示组件，`site.py` 只协调产物生成
- `src/research_school_radar/site_home_page.py`、`site_home.py` 与 `site_home_styles.py`：分别负责首页文档、说明区块和首页专属样式，协调器中不再保留页面模板
