...
公开层使用两套独立门槛。截止日期、至少四天、线下形式、有效标题、安全官网链接以及明确的费用或资助证据属于一票否决的公开条件；资金可及性是更严格的推荐条件。有资助或总费用不超过 400 EUR 的项目进入“资助优选”，费用明确且通过总价和日均上限的项目可以进入单独标注的“官网核实自费”层，但不会被描述成资助推荐。其他不确定记录仍留在内部审核队列。官网已核实的往届项目只进入明确标注已截止的周期项目库，不计入开放机会和 RSS。
...
首页对当前开放的“资助优选”和“官网核实自费”项目按每页 15 条分页。桌面端将搜索与筛选固定在左侧栏；手机端始终显示搜索，其他筛选通过按钮展开。截止日期、形式、时长、标题、链接或费用证据不明确的记录仍只保留在 scanner JSON 和审核队列中。往届周期项目使用独立、限量且按主办方均衡的项目库展示。
