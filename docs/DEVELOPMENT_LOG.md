...
## 2026-09-14: Repair responsive browsing and clarify page structure

The live UI audit reproduced zero-width title/location grid tracks on mobile, missing status labels after static localization stripped bilingual data attributes, hidden navigation/language links and coverage warnings, and filters that left archive cards visible below a zero-result message. Structural Python tests alone had not detected the rendered failures.
