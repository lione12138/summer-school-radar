...
## Responsive browsing and UI checks

The homepage separates open applications and past editions into independently searchable views. Search values, view selection and open-list pagination are encoded in URL parameters; reloads, language links and browser history preserve the selection. Both sections remain linked and available without JavaScript. Topic links appear before the results and supporting methodology is collapsed, with hash navigation reopening its containing disclosure.

Mobile cards use a single content column and real, localized status text rather than CSS-generated bilingual attributes. The shared native navigation menu retains language and theme controls. Coverage warnings remain visible at every width. Detail pages retain escaped original evidence in per-field disclosures; calendar menus share styles across page types. Source rows expose primary health fields and expandable coverage details, with search and health filters. None of these presentation changes modifies public eligibility or scanner records.

Optional browser validation: install the existing `render` extra and a Playwright Chromium runtime, then run `python scripts/check_site_ui.py --site-dir site`. `--browser-executable` selects an existing Chromium installation; `--screenshots` writes visual review artifacts. The check serves files only on loopback and tests the generated snapshot plus synthetic long-title and multi-page fixtures at 360, 390, 768 and 1440 pixels in both languages. It checks navigation, filtering, URL restoration, pagination, calendar/evidence interactions, source filters and light/dark button contrast. It is separate from the offline pytest suite and needs no API keys.

## Programme evidence and source coverage (2026-09-21)

A source can declare `evidence_urls` (programme/application/fees). These URLs are fetched explicitly in the existing bounded collection pass; this is not recursive crawling. With an explicit `programme_key`, `programme_evidence.py` merges a dated root record with registered same-edition pages. Programme dates, application conditions and costs have separate authoritative page roles. Prior editions and unrelated pages are excluded. `Candidate.evidence_sources` preserves field URLs through snapshots and the record audit. This deterministic enrichment does not alter the AI isolation policy.

ECMWF uses the complete Indico training catalogue; `training course` is a supported programme term. IAHS Academy discovery is limited to dated links on the official IAHS domain; its adapter retains the general fee and conditional reduced fee separately. Eurac reads its explicitly stated full two-week period instead of mistaking its two venues for optional sessions. Essex has one registry entry with an explicit application-page link. LASER is retained disabled because the official foundation says the school is paused.

Per-source `fetch_health` is distinct from `extraction_health` (`records_found`, `no_records`, `unavailable`). `extracted_records` counts pre-deduplication candidates; `scanner_records` remains the post-deduplication count. Consecutive empty scans are tracked for investigation, never treated as proof of a discovery miss. Legacy snapshots show extraction coverage as unmeasured. HTTP/browser challenge pages are failures, not healthy zero-result pages.

Windows task registration adds three failure retries at 15-minute intervals and a hidden PowerShell window. Browser-backed sources require `python -m playwright install chromium --only-shell` after Playwright upgrades. The 2026-09-21 interrupted task had no scan log; the exit status establishes interruption but not its external cause.


Sources may opt into `tls_trust: system` to use native truststore certificate validation (including linked evidence requests), without disabling TLS checks or modifying global SSL. SZN uses its official course catalogue and extracts only Joomla article content to exclude navigation dates. ESSLLI uses the official Tartu 2027 host with a scoped adapter separating instructor proposals/support from student registration; its annual host URL needs edition rollover review.


The registry also enables `slmath` and `crm_schools` from `collector_north_america.py`. SLMath reads official JSON with the JSON Accept header and uses a separately dated nomination-opening announcement. CRM follows at most 20 school-only activity routes and requires rendered detail markup. Both retain per-school dates and explicit identities; named SMS and CRM-PIMS probability editions share identity across the two catalogues. TRISEP and Perimeter use narrowly scoped page extraction. See `NORTH_AMERICA_SOURCES.md` for evidence and remaining unknown fields.


`collector_us_modules.py` adds registry-controlled `sismid` and `sisg` collectors. It reads individually selectable modules from dated tables, never concatenating their outer institute window. `us_school_adapters.py` repairs DL4SCI inline text and isolates ORNL NNS programme evidence. Fees expressed as ceilings remain non-exact; conditional registration waivers retain their restrictions. Incapsula error bodies are rejected as transport-content failures.


Australian collection uses registry-controlled `acspri` in `collector_acspri.py` and scoped AMSI Winter School/ACAN/ANSTO/UNSW Economics adapters in `australia_adapters.py`. ACSPRI preserves offering identity, counts explicit teaching dates, separates online/in-person variants and reads standard fees from their own price pages. Early-bird dates remain distinct from application closure. Exact adapter fees may use the configured currency conversion; explicit unknown amounts remain unknown. Adapter mode/status evidence is preserved. See `AUSTRALIA_SOURCES.md` for verification and the disabled AMSI Summer School date conflict.

ANSTO uses the EOI closing date rather than accepted-applicant registration. UNSW Economics full-programme dates and general fees cover both courses; individual half-programme durations are not inferred. Both entries use explicit programme identity.
Asian sources include the registry-controlled `icts_schools` collector in `collector_asia.py`: bounded official school-card discovery, scoped programme-body extraction, and per-detail error reporting. `asia_adapters.py` preserves AstroAI edition identity, venue and conditional funding. Access-blocked NUS IMS remains disabled. See `ASIA_SOURCES.md`.
`collector_neutron_japan.py` joins the 2026 Japanese Neutron and Muon School's official About/Application pages with mandatory edition, language, fee, extended-deadline and radiation-worker eligibility evidence. `asia_adapters.school_body` scopes VIASM details before generic exclusion and extraction; its adapter reads the actual title, labelled venue and explicit free registration. These entries require annual edition URL review.
`collector_ncts.py` is an edition-scoped NCTS QFT collector with explicit teaching-day count, registration window, temporary online delivery and conditional dormitory support. The CAMP adapter in `asia_adapters.py` recognizes the official intensive research course and separates its extended application deadline from the course dates.

`csha_adapter.py` recognizes only verified CSHA course IDs, reads banner identity/date fields and edition-labelled USD pricing, and preserves conditional stipend requests. It avoids treating sponsor logos as student aid or mixing USD/CNY amount pairs.


### Evidence-based financial and topic normalization (2026-09-25)

The extraction pipeline now separates source fields from a pure financial projection (`financial_normalization.py`) and evidence-scoped topic classification (`topic_taxonomy.py`). Raw pages remain in the HTTP cache; `fee_evidence`, `funding_evidence`, `evidence_sources` and `topic_evidence` retain the audit trail. This is an incremental middle layer, not a replacement crawler or a new fact database.

`FinancialTerms` separates stated/provisional prices, original-currency fee options and their conditions, offered/conditional support, included/covered accommodation and meals, and travel assistance. It is recomputed from the final candidate so merges and advisory edits cannot leave a stale qualification decision. Public API records include this clean projection; scanner evidence remains private. A conditional award alone does not qualify a record as funded or earn funding ranking points. The regular fee still determines low-fee or self-funded access. Clearly marked provisional official prices remain visible within the existing self-funded directory limits and enter review; they are not represented as confirmed prices. Original fee currencies and eligibility-dependent tiers appear in both languages.

Topics now have primary/secondary roles and supporting excerpts. Matching uses scoped programme content and word boundaries, excludes related-content containers, and distinguishes a satellite event from satellite research. A governance programme's AI content becomes secondary AI governance. Topic landing pages use primary topics. Snapshot v2 remains backward compatible through optional fields; older records fall back to legacy topics. DeepSeek topic additions must have resolved official evidence and pass the same context gate; generated summaries cannot supply evidence. The extraction prompt/cache version is `v8-financial-topic-evidence`.

The review queue records provisional fees, unsupported funding claims and conditional support needing clarification, alongside a separate field-completeness measure. Legacy extraction confidence remains a heuristic, not a calibrated correctness probability. The whole-record audit checks conditional-support/funded-status contradictions. Publication safety gates and scanner/display separation remain in force.

ALPS joins only explicitly registered same-edition root/application/fee pages. Current homepage corrections live in `data/overrides.yml` with `edition_year` guards; status refresh applies them without modifying full-scan snapshots. Subsequent full scans use the repaired adapters and classifiers. No new paid API call is required for the deterministic regression suite.
